...
const ActivitySlider = ({ isOpen, onClose, selectedDay, onActivitySelect }) => {
  ...
  // Update filtered activities to include search
  ...
  // Define handleActivitySelect inside the component
            ...
            <>
              {filteredActivities.slice(0, visibleItems).map((activity) => (
                <div key={activity.id} className="flex gap-4 border-b pb-4">
                  <BlurImage
                    src={activityImages[activity.id]?.[0] || 'https://placehold.co/300x200?text=Activity'}
                    alt={activity.title}
                  />
                  <div className="flex-1">
                  ...
                  <div className="text-right">
                    <div className="text-lg font-semibold">+ ₹{activity.discount_price}</div>
