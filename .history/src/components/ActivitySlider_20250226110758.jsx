...
const ActivitySlider = ({ isOpen, onClose, selectedDay, onActivitySelect }) => {
  ...
  const handleActivitySelect = useCallback((activity) => {
    if (!selectedDay || !onActivitySelect) return;
    ...
    const activityWithDay = {
      ...
      imageUrl: activityImages[activity.id]?.[0] || "https://placehold.co/300x200?text=Activity",
      price: activity.discount_price || activity.price
    };
    ...
    const fetchData = async () => {
      try {
        ...
        imagesData.Iamges.forEach(img => {
          ...
          imageMap[img.product_id].push(`https://backendactivity.plutotours.in/${img.image}`);
        });
