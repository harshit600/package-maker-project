...
const AllLeads = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      console.log("Fetching packages with token:", localUser.token); // Log token

      const response = await fetch(
        `${config.API_HOST}/api/packages/get-packages`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch packages: ${response.status} - ${rawResponse}`);
      }
