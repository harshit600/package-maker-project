...
const AllLeads = () => {
  ...
  const getFilteredPackages = () => {
    return packages.filter(pkg => {
      const matchesSearch = pkg.packageName?.toLowerCase().includes(packageFilter.search.toLowerCase()) ||
                           pkg.description?.toLowerCase().includes(packageFilter.search.toLowerCase());
      const matchesCategory = !packageFilter.category || pkg.packageCategory === packageFilter.category;
      const matchesDuration = !packageFilter.duration || pkg.duration === parseInt(packageFilter.duration);
      
      return matchesSearch && matchesCategory && matchesDuration;
    });
  };

  const uniqueCategories = [...new Set(packages.map(pkg => pkg.packageCategory))].filter(Boolean);
  const uniqueDurations = [...new Set(packages.map(pkg => pkg.duration))].filter(Boolean).sort((a, b) => a - b);
                        ...
                        <div className="flex flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4">
                          <div className="flex-1">
                            ...
                            <input
                              ...
                              onChange={(e) => setPackageFilter(prev => ({ ...prev, search: e.target.value }))}
                            />
                          </div>
                          ...
                          <div className="flex-1">
                            ...
                            <select
                              ...
                              onChange={(e) => setPackageFilter(prev => ({ ...prev, category: e.target.value }))}
                            >
                          ...
                          <div className="flex-1">
                            ...
                            <select
                              ...
                              onChange={(e) => setPackageFilter(prev => ({ ...prev, duration: e.target.value }))}
                            >
                          ...
                          <div className="flex-none self-end">
                            <button
                              onClick={() => setPackageFilter({ search: '', category: '', duration: '' })}
