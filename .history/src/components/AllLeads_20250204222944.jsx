  ...
  const handleShare = async (method) => {
    ...
    try {
      ...
      switch (method) {
        ...
        case 'email':
          const emailResponse = await fetch(`${config.API_HOST}/api/leads/send-package-email`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${localUser.token}`,
        ...
        case 'sms':
          const smsResponse = await fetch(`${config.API_HOST}/api/leads/send-package-sms`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${localUser.token}`,
    ...
    <div className="p-6 bg-white relative">
                          ...
                          <div className="grid grid-cols-3 gap-6">
                            ...
                            <button 
                              ...
                              className={`group relative bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md hover:border-green-500 transition-all duration-300 ${
                                isSharing && sharingMethod === 'whatsapp' ? 'opacity-75 cursor-wait' : ''
                              }`}
                            >
                              ...
                              <div className={`w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4 mx-auto group-hover:scale-110 transition-transform ${
                                isSharing && sharingMethod === 'whatsapp' ? 'animate-pulse' : ''
                              }`}>
                              ...
                              <p className="text-sm text-center text-gray-500">
                                {isSharing && sharingMethod === 'whatsapp' ? 'Opening WhatsApp...' : 'Share instantly via WhatsApp'}
                              </p>
                            </button>
      ...
      {isSharing && (
        <div className="fixed bottom-4 right-4 bg-white rounded-lg shadow-lg p-4 flex items-center space-x-3">
          ...
          <span className="text-sm text-gray-600">Sharing package via {sharingMethod}...</span>
        </div>
      )}
    </div>
  );
