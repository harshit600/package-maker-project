...
const AllLeads = () => {
  ...
  const getFilteredLeads = () => {
    return leads.filter((lead) => {
      const nameMatch = lead.name?.toLowerCase().includes(filters.name.toLowerCase());
      const destinationMatch = lead.destination?.toLowerCase().includes(filters.destination.toLowerCase());
      const durationMatch = !filters.duration || String(lead.days) === filters.duration;
      const packageCategoryMatch = !filters.packageCategory || lead.packageCategory === filters.packageCategory;
      const mobileMatch = lead.mobile?.toLowerCase().includes(filters.mobile.toLowerCase());
      
      const travelDateMatch = !filters.travelDate || (
        lead.travelDate && new Date(lead.travelDate).toISOString().split('T')[0] === filters.travelDate
      );

      return nameMatch && destinationMatch && durationMatch && 
             packageCategoryMatch && mobileMatch && travelDateMatch;
    });
  };
  ...
  return (
    <div className="p-3 bg-white relative">
      <div className="mb-4">
        <h2 className="text-lg font-semibold text-gray-800 mb-3">All Leads</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
          ...
          <div>
            ...
            >
              ...
              {getUniqueValues('days').sort((a, b) => a - b).map(days => (
          ...
          <div>
            <select
              ...
              onChange={(e) => setFilters(prev => ({ ...prev, packageCategory: e.target.value }))}
              className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="">All Categories</option>
              {getUniqueValues('packageCategory').map(category => (
      ...
      {isEditPanelOpen && (
        <>
          ...
          />
          <div className="fixed inset-y-0 right-0 w-1/2 bg-[rgb(45,45,68)] shadow-xl z-50 transform transition-transform duration-300 ease-in-out">
