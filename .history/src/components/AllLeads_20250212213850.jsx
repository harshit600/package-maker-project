...
const AllLeads = () => {
  ...
  const handleEditClick = async (lead) => {
    ...
    if (lead?.destination) {
      ...
      console.log(`Found ${packages.length} matching packages for ${lead.destination}`);
  ...
  // Add this function to fetch packages
  const fetchPackages = async () => {
    try {
      ...
      // Log the destination we're searching for
      console.log("Searching for destination:", selectedLead?.destination);
      ...
      // Filter packages where destination matches
      const filteredPackages = data.filter(pkg => {
        if (!selectedLead?.destination) return false;
        
        const searchTerm = selectedLead.destination.toLowerCase();
        const packageDest = (pkg.destination || '').toLowerCase();
        const packageName = (pkg.packageName || '').toLowerCase();
        const packageDesc = (pkg.description || '').toLowerCase();
        
        return packageDest.includes(searchTerm) || 
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-blue-50 p-4 rounded-lg">
                        ...
                        <p className="text-blue-700">
                          {selectedLead?.destination || "No destination selected"}
                        </p>
                      </div>
                        ...
                        {availablePackages
                          .filter((pkg) => {
                            const leadDestination =
                              selectedLead?.destination?.toLowerCase() || "";
                            const packageDestination =
                              pkg.destination?.toLowerCase() || "";
                            const packageName =
                              pkg.packageName?.toLowerCase() || "";

                            // Show package if destination appears in package name or destination field
                            return (
                              packageDestination.includes(leadDestination) ||
                              packageName.includes(leadDestination)
                            );
                          })
                          .map((pkg) => (
                            <div
                      ...
                      {availablePackages.filter((pkg) => {
                        ...
                        <div className="text-center py-8">
                          <p className="text-gray-500">
                            ...
                            {selectedLead?.destination || "this destination"}
                          </p>
                        </div>
                      )}
                    </div>
