                          ...
                          {selectedPackage && (
                            <>
                              ...
                              {packageInfoTab === "cab" && (
                                <div className="space-y-6">
                                  {/* Header Card */}
                                  <div className="bg-gradient-to-br from-[rgb(45,45,68)] to-[rgb(55,55,88)] rounded-xl p-8 text-white">
                                    <div className="flex items-center justify-between">
                                      <div className="flex items-center gap-4">
                                        <div className="p-3 bg-white/10 rounded-xl">
                                          <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M21.739 10.921c-1.347-.39-1.885-.538-3.552-.921 0 0-2.379-2.359-2.832-2.816-.568-.572-1.043-1.184-2.949-1.184h-7.894c-2.5 0-2.5 1.927-2.5 1.927l-.001 1.964c0 .766.336 1.614.968 2.074l.001.002c.021.027 1.006 1.633 1.006 1.633.102 1.57.338 2.28 2.557 2.28h.001c2.247 0 2.354-1.038 2.354-2.28l-.001-.839h4.159l-.001.839c0 1.242-.107 2.28 2.354 2.28 2.214 0 2.455-.71 2.557-2.28l1.007-1.633c.631-.46.967-1.308.967-2.074v-1.964c0-.766-.336-1.614-.968-2.074l-.001-.002zm-16.575 1.515c-.44 0-.808-.361-.808-.803 0-.441.368-.803.808-.803s.808.362.808.803c0 .442-.368.803-.808.803zm12.597.803c0 .441-.368.803-.808.803s-.808-.361-.808-.803c0-.441.368-.803.808-.803s.808.362.808.803z"/>
                                          </svg>
                                        </div>
                                        <div>
                                          <h3 className="text-2xl font-bold mb-1">Vehicle Selection</h3>
                                          <p className="text-white/80">Choose your perfect ride for each day</p>
                                        </div>
                                      </div>
                                      <div className="bg-white/10 rounded-lg p-4 text-center">
                                        <p className="text-sm text-white/80 mb-1">Total Days</p>
                                        <p className="text-2xl font-bold">{selectedPackage.package?.itineraryDays?.length || 0}</p>
                                      </div>
                                    </div>
                                  </div>

                                  {/* Day-wise Cab Selection */}
                                  {selectedPackage.package?.itineraryDays?.map((day, dayIndex) => (
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                          <div>
                                            <div className="flex items-center gap-3 mb-2">
                                              ...
                                              <span className="text-gray-500 text-sm">
                                                {new Date(day.selectedItinerary?.date).toLocaleDateString('en-US', {
                                                  weekday: 'long',
                                                  month: 'long',
                                                  day: 'numeric'
                                                })}
                                          ...
                                          <div className="flex items-center gap-6">
                                            ...
                                            <div className="flex items-center gap-2">
                                              ...
                                              <span className="text-sm font-medium text-gray-700">
                                                {day.selectedItinerary?.totalHours} hours
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                          {Object.entries(selectedPackage.cabs?.travelPrices?.selectedCabs || {}).map(([cabType, cabs]) => (
                                            cabs.map((cab, cabIndex) => (
                                              <label
                                                key={`${cabType}-${cabIndex}`}
                                                className={`relative flex flex-col h-full cursor-pointer rounded-xl border-2 p-4 transition-all
                                                  ${day.selectedCab?.type === cabType && day.selectedCab?.cabName === cab.cabName
                                                ...
                                                <input
                                                  ...
                                                  onChange={() => {
                                                    const updatedPackage = {
                                                      ...
                                                      package: {
                                                        ...selectedPackage.package,
                                                        itineraryDays: selectedPackage.package.itineraryDays.map((d, i) =>
                                                          i === dayIndex
                                                            ? { ...d, selectedCab: { type: cabType, ...cab } }
                                                            : d
                                                        ),
                                                      },
                                                    };
                                                  ...
                                                  checked={day.selectedCab?.type === cabType && day.selectedCab?.cabName === cab.cabName}
                                                />

                                                <div className="flex-1">
                                                  <div className="mb-3">
                                                    <h5 className="text-lg font-semibold text-gray-900 mb-1">{cab.cabName}</h5>
                                  ...
                                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                                    <div className="flex items-center justify-between">
                                      <div>
                                        <h5 className="text-lg font-semibold text-gray-900">Total Transportation Cost</h5>
                                        <p className="text-sm text-gray-500 mt-1">
                                          For {selectedPackage.package?.itineraryDays?.length || 0} days
                                        </p>
                                      </div>
                                      <div className="text-right">
                                        <p className="text-3xl font-bold text-green-600">
                                          ₹{selectedPackage.package?.itineraryDays?.reduce((total, day) => {
                                            return total + (day.selectedCab?.prices?.onSeasonPrice || 0);
                                          }, 0)}
                                        </p>
