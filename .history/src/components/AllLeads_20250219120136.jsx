...
const AllLeads = () => {
  ...
  // Add new state for all available hotels
  ...
  // Add new function to fetch all hotels
  ...
  // Add function to get hotels for a specific city
  const getHotelsForCity = (cityName) => {
    if (!cityName) return [];
    return allHotels.filter(hotel => 
      hotel.location?.city?.toLowerCase() === cityName.toLowerCase()
    );
  };
                                ...
                                {selectedPackage?.package?.itineraryDays?.map(
                                  (day, index) => (
                                    <div key={index} className="mb-8 last:mb-0">
                                      ...
                                      {/* Add the hotels section */}
                                      {day.selectedItinerary.cityName && (
                                        <div className="mt-4">
                                          <div className="flex justify-between items-center mb-3">
                                            <h4 className="text-lg font-medium text-gray-900">Available Hotels in {day.selectedItinerary.cityName}</h4>
                                            <button
                                          ...
                                          <div className="space-y-4">
                                            {getHotelsForCity(day.selectedItinerary.cityName).map((hotel, idx) => (
                                              <div key={idx} className="bg-gray-50 rounded-lg p-4">
                                                <div className="flex gap-4">
                                                  <div className="w-32 h-24 bg-gray-200 rounded-lg overflow-hidden">
                                                    {hotel.images?.[0] ? (
                                                      <img
                                                        src={`${config.API_HOST}/${hotel.images[0]}`}
                                                        alt={hotel.name}
                                                        className="w-full h-full object-cover"
                                                      />
                                                    ) : (
                                                  ...
                                                  <div className="flex-1">
                                                    <div className="flex justify-between items-start">
                                                      <div>
                                                        ...
                                                        <p className="text-sm text-gray-600 mt-1">{hotel.location?.address}</p>
                                                      </div>
                                                      <div className="text-right">
                                                        <div className="flex items-center gap-1">
                                            ...
                                            {getHotelsForCity(day.selectedItinerary.cityName).length === 0 && (
                                              <p className="text-gray-500 text-center py-4">
                                                No hotels available in {day.selectedItinerary.cityName}
                                              </p>
                                            )}
                                          </div>
                                        </div>
