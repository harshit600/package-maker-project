...
const AllLeads = () => {
  ...
  useEffect(() => {
    console.log("Current allHotels state:", allHotels);
  }, [allHotels]);
  ...
  const fetchHotels = async () => {
    try {
      const response = await fetch(`${config.API_HOST}/api/packagemaker/get-packagemaker-hotels-by-city`);
      const data = await response.json();
      
      if (data.success) {
        // Organize hotels by city
        const hotelsByCityMap = data.data.reduce((acc, hotel) => {
        ...
        console.log('Hotels organized by city:', hotelsByCityMap);
      }
    } catch (error) {
  ...
  // Modify the getHotelsForCity function to check for city mentions
  const getHotelsForCity = (itineraryText) => {
    if (!itineraryText) return [];

    // Log for debugging
    console.log("Checking itinerary text:", itineraryText);
    console.log("Available hotels:", allHotels);

    // Check for Shimla mention in the text
    if (itineraryText.toLowerCase().includes("shimla")) {
      const shimlaHotels = allHotels.filter(
        (hotel) => hotel.location?.city?.toLowerCase() === "shimla"
      );
      console.log("Found Shimla hotels:", shimlaHotels);
      return shimlaHotels;
    }
  ...
  // Function to extract city name from itinerary
  const extractCityFromItinerary = (itinerary) => {
    if (!itinerary) return null;
    
    // Common cities to look for
    const cities = ['Dharamshala', 'Shimla', 'Manali', 'Dalhousie'];
    
    // Convert text to lowercase for case-insensitive matching
    const text = (itinerary.itineraryTitle + ' ' + itinerary.itineraryDescription).toLowerCase();
    
    // Find the first matching city
    const foundCity = cities.find(city => text.includes(city.toLowerCase()));
    return foundCity;
  };
  if (loading) {
    return (
