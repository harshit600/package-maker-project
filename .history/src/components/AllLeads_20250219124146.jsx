...
const AllLeads = () => {
  ...
  // Add this state to store hotels for each city
  ...
  const fetchHotelsByCity = async (cityName) => {
    try {
      ...
      const response = await fetch(`${config.API_HOST}/api/packagemaker/get-packagemaker-hotels-by-city/${cityName}`);
      const data = await response.json();
      console.log(`Hotels data for ${cityName}:`, data);
                                ...
                                {selectedPackage?.package?.itineraryDays?.map(
                                  (day, index) => (
                                    <div key={index} className="mb-8 last:mb-0">
                                      ...
                                      {/* Add this effect to fetch hotels when itinerary is shown */}
                                      {useEffect(() => {
                                        if (day.selectedItinerary?.itineraryTitle) {
                                          // Extract city name from itinerary title
                                          const cityName = day.selectedItinerary.itineraryTitle.split('-')[0].trim();
