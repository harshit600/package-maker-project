...
const AllLeads = () => {
  ...
  const fetchhotelcity = async () => {
    let cityNamee = "Shimla";
    try {
      const response = await fetch(
        `${config.API_HOST}/api/packagemaker/get-packagemaker-hotels-by-city/${cityNamee}`
      );
      const data = await response.json();
      console.log("Hotels data for Shimla:", data);
    } catch (error) {
      toast.error(error.message);
    }
  };
