...
const CabCard = ({ cab, onEdit, onDelete }) => {
  ...
  const handleEdit = () => {
    console.log("Editing cab with data:", cab); // Debug log
    // Make sure all required data is present before calling onEdit
    if (!cab || !cab._id) {
      console.error("Invalid cab data for editing:", cab);
      return;
    }
    onEdit({
      _id: cab._id,
