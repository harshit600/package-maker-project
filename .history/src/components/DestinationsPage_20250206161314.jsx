...
const DestinationPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-12 w-full">
      <div className="w-full px-6 sm:px-8 lg:px-12">
        <div className="bg-white rounded-xl shadow-lg p-8 w-full">
          <div className="flex justify-between items-center mb-8 w-full">
            ...
            <Button
              ...
              variant="primary"
              className="hover:shadow-lg transition-all duration-200 px-6 py-2.5"
            />
          </div>

          {loading ? (
            <div className="flex justify-center items-center h-72 w-full">
              <BeatLoader color="#4F46E5" loading={true} size={12} />
            </div>
          ) : (
            <Table
              ...
              renderRow={(country) => (
                <>
                  <td className="py-5 px-8 border-b border-slate-200 w-3/4 text-slate-700">
                    {country.countryName}
                  </td>
                </>
              )}
              renderActions={(country) => (
                <td className="py-5 px-8 border-b border-slate-200 w-1/4">
        ...
        <Modal
          ...
          className="w-full"
        >
          <Modal.Header closeButton className="border-b border-slate-200 p-6">
            <Modal.Title className="text-2xl font-bold text-slate-800">
          ...
          <Modal.Footer className="border-t border-slate-200 p-6">
            <div className="flex gap-4">
              <Button
                variant="secondary"
                text="Cancel"
                onClick={handleCloseModal}
                className="px-6 py-2.5 text-slate-700 hover:bg-slate-100 transition-colors duration-200"
              />
              <Button
                variant="primary"
                text="Save"
                onClick={handleSubmit}
                className="px-6 py-2.5 hover:shadow-lg transition-all duration-200"
              />
            </div>
          </Modal.Footer>
        </Modal>
