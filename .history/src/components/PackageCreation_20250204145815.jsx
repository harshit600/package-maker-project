...
const PackageCreation = ({ initialData, isEditing, editId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!data || (!data.data && !data.package)) {
        throw new Error("Invalid server response: No data received");
      }

      // Get the package data from either data.data or data.package
      const packageResponse = data.data || data.package;

      // Store the package ID in cabsData for next steps
      setCabsData({
        ...packageResponse,
        packageId: packageResponse._id || editId,
