...
const PackageCreation = ({ initialData, isEditing, editId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      console.log("Server response:", data); // Let's see what the server sends back
      ...
      // Get the package data from the response
      const packageResponse = data.data || data.package || data;

      // Verify we have the necessary data
      if (!packageResponse || !packageResponse._id) {
        throw new Error("Invalid package data received from server");
      }

      // Set the cabs data with the response
      setCabsData({
        ...packageResponse,
