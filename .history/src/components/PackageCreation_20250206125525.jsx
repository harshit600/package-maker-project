...
const PackageCreation = ({ initialData, isEditing, editId }) => {
    ...
    try {
      ...
      if (!query) {
        ...
        if (dayInfo.type === "travel") {
          // For travel days, search for itineraries between cities
          searchQuery = `${dayInfo.from} to ${dayInfo.to}${
            dayInfo.isNightTravel ? " night" : ""
          }`;
        } else if (dayInfo.type === "local") {
          // For local days, search for local sightseeing
      ...
      // Fetch itineraries from API with detailed parameters
      const response = await fetch(
        `${config.API_HOST}/api/itinerary/searchitineraries?search=${encodeURIComponent(searchQuery)}&type=${sequence[index].type}&from=${encodeURIComponent(sequence[index].from || '')}&to=${encodeURIComponent(sequence[index].to || '')}&location=${encodeURIComponent(sequence[index].location || '')}`
      );
      ...
      // Update itinerary days with search results and additional details
      setItineraryDays((prevDays) => {
        ...
        // If we have results, update the selected itinerary with full details
        if (data.length > 0) {
  ...
  // 3. useEffect that triggers initialization when package details are complete
  useEffect(() => {
    if (
      formData.duration &&
      ...
      if (totalNights === maxNights) {
        // If editing and have initial data, use that first
