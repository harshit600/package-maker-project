...
const PackageCreation = ({ initialData, isEditing, editId }) => {
  ...
  const initializeItineraryDays = async () => {
    ...
    for (let i = 0; i < days.length; i++) {
      ...
      console.log(`Fetching itinerary for day ${i + 1} with query:`, searchQuery);

      try {
        const response = await fetch(
          `${config.API_HOST}/api/itinerary/searchitineraries?search=${encodeURIComponent(searchQuery)}&type=${day.type}&from=${encodeURIComponent(day.from || '')}&to=${encodeURIComponent(day.to || '')}&location=${encodeURIComponent(day.location || '')}`
        );
  ...
  }, [packagePlaces, formData.duration, formData.pickupLocation, formData.dropLocation]);
