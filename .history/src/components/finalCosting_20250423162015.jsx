...
// Update the PackagePDF component to handle image loading
const PackagePDF = ({ packageSummary, showMargin, marginAmount, finalTotal, getCurrentMarginPercentage }) => {
  const [loadedImages, setLoadedImages] = useState({});

  useEffect(() => {
    const loadImages = async () => {
      const images = {};
      for (const hotel of packageSummary.hotels || []) {
        if (hotel.propertyphoto && Array.isArray(hotel.propertyphoto)) {
          images[hotel.day] = await Promise.all(
            hotel.propertyphoto.map(url => fetchImage(url))
          );
        }
        if (hotel.roomimage) {
          images[`room_${hotel.day}`] = await fetchImage(hotel.roomimage);
        }
      }
