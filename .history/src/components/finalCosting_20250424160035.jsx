...
const FinalCosting = () => {
  ...
  useEffect(() => {
    const fetchLeadDetails = async () => {
      try {
        const response = await fetch(`${config.API_HOST}/api/leads/get`);
        if (!response.ok) {
          throw new Error('Failed to fetch leads');
        }
        const leadsData = await response.json();
        
        // Filter lead for current package
        const matchingLead = leadsData.find(lead => lead.id === packageSummary?.id);
        
        if (matchingLead) {
          console.log('Matching Lead Details:', matchingLead);
          setLeadDetails(matchingLead);
        } else {
          console.log('No matching lead found for package ID:', packageSummary?.id);
        }
