  ...
  useEffect(() => {
    const fetchLeadData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3001/api/leads/get');
        if (!response.ok) {
          throw new Error('Failed to fetch leads');
        }
        const leadsData = await response.json();
        
        // Filter for the lead that matches the package
        const matchingLead = leadsData.find(lead => lead.id === packageSummary.leadId);
        
        if (matchingLead) {
          console.log('Selected Lead Data:', matchingLead);
    ...
    if (packageSummary?.leadId) {
      fetchLeadData();
    }
  }, [packageSummary?.leadId]);
