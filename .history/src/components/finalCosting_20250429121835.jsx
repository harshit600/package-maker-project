    ...
    <Page size="A4" style={pdfStyles.page}>
      ...
      <View style={pdfStyles.mainContent}>
          ...
          >
            <View style={pdfStyles.rowBetween}>
              <View>
                ...
                <Text style={{ fontSize: 10, color: "#E2E8F0", marginTop: 4 }}>
                  Including all taxes
                </Text>
              </View>
              <Text
              ...
              >
                ₹{packageSummary.totals.grandTotal.toLocaleString()}
              </Text>
            </View>
          </View>
        ...
        >
          ...
          >
            <Icons.Hotel
              style={{ width: 16, height: 16, color: "rgb(45 45 68)" }}
            />
            <Text
              style={{
                fontSize: 14,
          ...
          {packageSummary.hotels.map((hotel, index) => (
            <View
              ...
              style={{
                marginBottom:
                  index === packageSummary.hotels.length - 1 ? 0 : 20,
                backgroundColor: "#F8FAFC",
                borderRadius: 8,
                overflow: "hidden",
              }}
            >
              ...
              <View style={{ padding: 12 }}>
                ...
                >
                  {hotel.propertyphoto &&
                    hotel.propertyphoto.slice(0, 3).map((photo, photoIndex) => (
                      <PDFImage
                        key={photoIndex}
                        source={{ uri: photo }}
                        style={{
...
const FinalCosting = ({ selectedLead }) => {
  ...
  const [processedHotels, setProcessedHotels] = useState([]);
  ...
  useEffect(() => {
    const processImages = async () => {
      const processed = await Promise.all(
        packageSummary.hotels.map(async (hotel) => {
          // Process room image
          const roomImageBase64 = await getBase64FromUrl(hotel.roomimage);
          
          // Process property photos
          const propertyPhotosBase64 = await Promise.all(
            hotel.propertyphoto.slice(0, 3).map(photo => getBase64FromUrl(photo))
          );

          return {
            ...hotel,
            roomImageBase64,
            propertyPhotosBase64: propertyPhotosBase64.filter(Boolean)
          };
        })
      );
