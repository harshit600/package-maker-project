          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-300">Previous quotations sent</p>
              </div>
            </div>
          </div>
              ...
              {history.length === 0 ? (
                <div className="text-center py-8">
                  <div className="bg-gray-50 rounded-xl p-6">
                    ...
                    <p className="text-gray-600 font-medium">No Quote History</p>
                    <p className="text-sm text-gray-500 mt-1">Send your first quote to get started</p>
                  </div>
                </div>
              ) : (
                history
                  ...
                  .map((item, index) => (
                    <div key={index} className="bg-white rounded-xl border border-gray-100 p-4 hover:shadow-lg transition-all duration-300">
                      {/* Quote Header */}
                      <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-100">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-sm font-medium text-gray-900">Quote #{index + 1}</p>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        ...
                        <div className="bg-blue-50 rounded-lg p-3">
                          ...
                          <p className="text-sm font-semibold text-blue-900">₹{item.finalTotal.toLocaleString()}</p>
                        </div>
                        <div className="bg-purple-50 rounded-lg p-3">
                          <p className="text-xs text-gray-500">Margin</p>
                          <p className="text-sm font-semibold text-purple-900">{item.marginPercentage}%</p>
                        </div>
                        <div className="bg-green-50 rounded-lg p-3">
                          <p className="text-xs text-gray-500">Discount</p>
                          <p className="text-sm font-semibold text-green-900">{item.discountPercentage}%</p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <PDFDownloadLink
                            document={<PlutoToursPDF packageSummary={{...item, package: packageSummary.package}} showMargin={true} marginAmount={item.finalTotal - item.total} finalTotal={item.finalTotal} getCurrentMarginPercentage={() => item.marginPercentage} selectedLead={selectedLead} />}
                            fileName={`pluto-tours-package-${item.id}-${new Date(item.timestamp).getTime()}.pdf`}
                          >
                            {({ loading }) => (
                              <button className="w-full px-3 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-sm font-medium">
                          ...
                          <PDFDownloadLink
                            document={<DemandSetuPDF packageSummary={{...item, package: packageSummary.package}} showMargin={true} marginAmount={item.finalTotal - item.total} finalTotal={item.finalTotal} getCurrentMarginPercentage={() => item.marginPercentage} selectedLead={selectedLead} />}
                            fileName={`demand-setu-package-${item.id}-${new Date(item.timestamp).getTime()}.pdf`}
                          >
                            {({ loading }) => (
                              <button className="w-full px-3 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-sm font-medium">
                        ...
                        <button
                          onClick={async () => {
                            try {
                              const response = await fetch(`${config.API_HOST}/api/finalcosting/update-entire/${item._id}`, {
                                method: "PUT",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({ ...item, converted: true }),
                              });

                              if (!response.ok) throw new Error("Failed to update history");

                              const updatedHistory = history.map((historyItem) =>
                                historyItem._id === item._id ? { ...historyItem, converted: true } : historyItem
                              );
                          ...
                          className={`w-full px-3 py-2 ${item.converted ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"} text-white rounded-lg transition-colors text-sm font-medium`}
                          disabled={item.converted}
                        >
                          {item.converted ? "Quote Converted" : "Convert Quote"}
                        </button>
                      </div>
                    </div>
                  ))
