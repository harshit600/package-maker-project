...
import { CalendarDaysIcon, ChevronLeftIcon, ChevronDownIcon, UserIcon } from "@heroicons/react/24/outline"
...
export default function BulkUpdateTabs() {
  ...
  const toggleDay = (index) => {
    ...
    setSectionData((prevData) => ({
      ...
      [activeTab]: {
        ...prevData[activeTab],
        selectedDays: prevData[activeTab].selectedDays.map((day, i) => (i === index ? !day : day)),
      },
    }))
  }
...
useEffect(() => {
    ...
    <Card className="bg-white shadow-md">
      ...
      <CardBody>
        <div className="flex justify-between gap-6">
          {Object.keys(sectionData.inventory).filter(key => key !== 'dateRange' && key !== 'selectedDays').map((room, index) => (
            <div key={index}>
              ...
              <Input
                ...
                onChange={(e) => handleInputChange("inventory", room, e.target.value)}
              />
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-4">
          <Checkbox
            ...
            onChange={() => handleInputChange("rates", "showNettRate", !sectionData.rates.showNettRate)}
          />
          <Checkbox
            label="Update Extra Guest Charges"
            checked={sectionData.rates.updateExtraCharges}
            onChange={() => handleInputChange("rates", "updateExtraCharges", !sectionData.rates.updateExtraCharges)}
          />
        </div>
      </div>

      {Object.entries(sectionData.rates.rooms).map(([room, mealPlans], index) => (
        <Accordion
          key={index}
                ...
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(occupancies).map(([occupancy, rate]) => (
                    <div key={occupancy} className="flex items-center gap-2">
                      ...
                      <div className="w-10 h-10 bg-gray-100 flex items-center justify-center rounded">{occupancy}</div>
                      <Input
                        type="number"
                        placeholder="Enter rate"
                        className="!border !border-gray-300"
                        value={rate}
                        onChange={(e) => handleRateChange(room, mealPlan, occupancy, e.target.value)}
                      />
                    </div>
                  ))}
                </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="flex flex-col space-y-2">
          <Checkbox
            ...
            onChange={() => handleInputChange("restrictions", "blockInventory", !sectionData.restrictions.blockInventory)}
          />
          <Checkbox
            label="Unblock Inventory"
            checked={sectionData.restrictions.unblockInventory}
            onChange={() => handleInputChange("restrictions", "unblockInventory", !sectionData.restrictions.unblockInventory)}
          />
        </div>
        <div className="flex flex-col space-y-2">
          <Checkbox
            ...
            onChange={() => handleInputChange("restrictions", "closeToArrival", !sectionData.restrictions.closeToArrival)}
          />
          <Checkbox
            label="Inactivate CTA"
            checked={sectionData.restrictions.inactivateCTA}
            onChange={() => handleInputChange("restrictions", "inactivateCTA", !sectionData.restrictions.inactivateCTA)}
          />
        </div>
        <div className="space-y-2">
          <Checkbox
            ...
            onChange={() => handleInputChange("restrictions", "closeToDeparture", !sectionData.restrictions.closeToDeparture)}
          />
          <Checkbox
            label="Inactivate CTD"
            checked={sectionData.restrictions.inactivateCTD}
            onChange={() => handleInputChange("restrictions", "inactivateCTD", !sectionData.restrictions.inactivateCTD)}
          />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            onChange={(e) => handleInputChange("restrictions", "minimumLengthOfStay", e.target.value)}
          />
        </div>
        <div>
          <Typography variant="small" color="blue-gray" className="mb-2">
  ...
  return (
    <div className="w-[80%] max-w-[1600px] mx-auto p-4">
        ...
        <Button variant="text" className="flex items-center gap-2 text-blue-500 p-0 mb-4" size="sm"  onClick={() => navigate(-1)} >
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <Typography variant="small" color="blue-gray">
                Selected days ({sectionData[activeTab].selectedDays.filter(Boolean).length})
              </Typography>
