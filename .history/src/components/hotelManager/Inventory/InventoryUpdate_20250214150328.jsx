  ...
  const handleRateChange = (roomName, rateName, occupancy, dayIndex, value) => {
    ...
    setHotelData((prevData) => {
      ...
      const updatedRates = currentRates.map((rate, index) => {
        ...
        if (rate === null) {
          return {
            date: new Date(Date.now() + index * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Generate a date for the future
            value: null, // Default value
          };
        }
              ...
              {tabs.map(({ value }) => (
                <TabPanel key={value} value={value}>
                  <div className="flex flex-wrap gap-4 mb-4">
                    ...
                    <Button onClick={modalHandler} variant="outlined">Bulk Update</Button>
                    <IconButton variant="outlined" color="blue-gray" size="sm" onClick={() => changeWeek('prev')}>
                      <ChevronLeftIcon className="h-4 w-4" />
                    </IconButton>
                    <IconButton variant="outlined" color="blue-gray" size="sm">
                      <CalendarIcon className="h-4 w-4" />
                    </IconButton>
                    <IconButton variant="outlined" color="blue-gray" size="sm" onClick={() => changeWeek('next')}>
                    ...
                    <table className="w-full min-w-max table-auto text-left">
                      <thead>
                        <tr>
                          ...
                          {dates.map((date) => (
                            <th key={date} className="border-b border-gray-300 text-center p-2">
                        ...
                        {roomData && updatedRoomTypes[activeTab].map((roomType, index) => (
                          <React.Fragment key={index}>
                            ...
                            <tr className="hover:bg-blue-gray-50/50 cursor-pointer">
                              <td className="p-4 border-b border-blue-gray-50" onClick={() => toggleRoomExpansion(roomType.name)}>
                                <div className="flex items-center gap-2">
                              ...
                              {hotelData[activeTab]?.[roomType.name]?.availability?.map((day, dayIndex) => (
                                <td key={dayIndex} className="p-2 border-b border-blue-gray-50">
                                  ...
                                  <>
                                    {[1, 2].map((occupancy) => (
                                      <tr key={occupancy} className="bg-gray-50/50">
                                        ...
                                        )) || Array(7).fill(<td className="p-2 border-b border-gray-200"><input type="number" value={""} className="w-full text-center font-bold p-1 border border-gray-300 rounded text-xs bg-white" /></td>)}
                                      </tr>
                                    ))}
                                    {console.log(hotelData[activeTab]['Deluxe Room'])}
                                  </>
                                )}
                              </React.Fragment>
                            ))}
