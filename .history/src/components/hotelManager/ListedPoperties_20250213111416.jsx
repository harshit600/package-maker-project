...
function RoomListing() {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this room?')) {
            try {
                const response = await fetch(`${config.API_HOST}/api/rooms/delete-room/${id}`, {
                    method: 'DELETE'
                });
                const data = await response.json();
                
                if (data.success) {
                    setRoomsData(prevData => prevData.filter(room => room._id !== id));
                }
            } catch (error) {
    ...
    return (
        <div className='p-4'>
            {roomsData?.map((room) => (
                <div key={room._id} className='property-card flex justify-between mt-4 hover:bg-gray-50 transition-all duration-300 p-4 rounded-lg'>
                    <div className='flex gap-8 items-center'>
                        <img 
                            src={room?.images?.[0]} 
                            height='200px' 
                        ...
                        <div className='flex flex-col'>
                            <div className='font-bold text-2xl text-gray-800'>{room?.roomName}</div>
                            <div className='font-normal text-gray-600 mt-2'>
                                Room Type: {room?.roomType} | Floor: {room?.floor} | 
