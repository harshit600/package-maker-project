...
function ListedProperties() {
  ...
  const locations = [...new Set(propertiesData?.map(p => p?.location?.city) || [])];
  
  const filteredProperties = propertiesData?.filter(property => {
    const matchesSearch = property?.basicInfo?.propertyName
      .toLowerCase()
      .includes(filters.search.toLowerCase());
    const matchesStars = filters.stars === "all" || property?.basicInfo?.stars === parseInt(filters.stars);
    const matchesLocation = filters.location === "all" || property?.location?.city === filters.location;
      ...
      <div className="space-y-6">
        {filteredProperties?.length === 0 ? (
          <div className="text-center py-12">
            <div className="text-gray-400 text-lg">No properties found matching your filters</div>
          </div>
        ) : (
