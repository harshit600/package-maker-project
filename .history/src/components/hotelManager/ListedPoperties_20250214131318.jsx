...
function ListedProperites({
  ...
  // Add new function to filter properties
    ...
    return propertiesData
      .filter((property) => {
        ...
        if (priceFilter !== "All Prices") {
          switch (priceFilter) {
            case "Under ₹1000":
              priceMatch = lowestPrice < 1000;
              break;
            case "₹1000 - ₹5000":
              priceMatch = lowestPrice >= 1000 && lowestPrice <= 5000;
              break;
            case "Above ₹5000":
              priceMatch = lowestPrice > 5000;
              break;
          }
        }
        ...
        if (ratingFilter !== "All Ratings") {
          const propertyRating = property?.basicInfo?.starRating || 0;
          ratingMatch = propertyRating === parseInt(ratingFilter);
        }
      ...
      .sort((a, b) => {
        ...
        switch (sortBy) {
          case "rating_high":
            return (
              (b?.basicInfo?.starRating || 0) - (a?.basicInfo?.starRating || 0)
            );
          case "price_high":
            return (
              Math.min(
                ...(b?.rooms?.data?.map((room) => parseInt(room.baseRate)) || [
                  0,
                ])
              ) -
              Math.min(
                ...(a?.rooms?.data?.map((room) => parseInt(room.baseRate)) || [
                  0,
                ])
              )
            );
          ...
          default: // "popular" or any other case
            return 0; // No sorting
        }
      });
  };
