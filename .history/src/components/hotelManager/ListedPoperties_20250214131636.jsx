...
function ListedProperites({
  ...
  useEffect(() => {
    if (propertiesData) {
      ...
      const locations = [...new Set(propertiesData.map(property => 
        property?.location?.address
      ).filter(Boolean))];

      // Get unique ratings
      const ratings = [...new Set(propertiesData.map(property => 
        property?.basicInfo?.starRating
      ).filter(Boolean))].sort((a, b) => b - a);

      // Calculate price ranges based on room rates
      const allPrices = propertiesData.flatMap(property =>
      ...
      const priceRanges = [
        ...
        { label: `₹${Math.floor(minPrice + 1000)} - ₹${Math.floor(maxPrice - 1000)}`, value: "mid_range" },
      ...
      setFilterOptions({
        ...
        priceRanges,
        ratings: ["All Ratings", ...ratings.map(rating => `${rating} Star${rating > 1 ? 's' : ''}`)]
      });
    }
  }, [propertiesData]);
  ...
  const getFilteredProperties = () => {
    ...
    return propertiesData.filter(property => {
      ...
      // Rating filter
      const ratingMatch = ratingFilter === "All Ratings" || 
