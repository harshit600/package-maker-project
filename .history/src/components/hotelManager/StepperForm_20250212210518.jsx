...
const tabStyles = {
  wrapper: "relative flex overflow-x-auto border-b border-gray-200 mt-5 mb-8 scrollbar-hide",
  progressBar: "absolute bottom-0 left-0 h-1 bg-blue-500 transition-all duration-300",
  tab: `
    relative min-w-[140px] flex items-center gap-3 px-6 py-4 
  ...
  label: "font-medium whitespace-nowrap text-sm",
  connector: "absolute right-0 top-1/2 -translate-y-1/2 w-8 h-[2px] bg-gray-200"
};
      ...
      <div className={tabStyles.wrapper}>
        ...
        {steps.map((step, index) => (
          <div
            ...
            className={`
              ${tabStyles.tab}
              ${activeStep === index ? tabStyles.tabActive : tabStyles.tabInactive}
            `}
          >
            <div 
              className={`
                ${tabStyles.icon}
                ${activeStep >= index ? tabStyles.iconActive : tabStyles.iconInactive}
              `}
            >
              {index < activeStep ? (
            ...
            <span className={tabStyles.label}>{step.name}</span>
            {index < steps.length - 1 && (
              <div className={tabStyles.connector} />
            )}
          </div>
        ))}
      </div>
