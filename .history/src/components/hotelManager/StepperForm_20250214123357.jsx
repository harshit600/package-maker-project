                ...
                return (
                  <div key={step.name} className="flex items-center">
                    <button
                      ...
                      className={`group flex flex-col items-center px-6 py-3 relative
                        ${isClickable ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}
                        ${isActive ? 'text-blue-600' : isPast ? 'text-green-600' : 'text-gray-500'}`}
                    >
                      {/* Step connector line */}
                      {index < steps.length - 1 && (
                        <div className={`absolute top-[2.75rem] left-[4.5rem] w-[calc(100%-2rem)] h-0.5 
