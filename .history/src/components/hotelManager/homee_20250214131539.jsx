      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white mt-6 p-4 rounded-lg shadow">
          <div className="flex items-center gap-6">
            ...
            <Menu>
              <MenuHandler>
                <Button variant="text" className="flex items-center gap-2 text-gray-700">
                  {selectedPrice} <ChevronDownIcon strokeWidth={2} className="h-4 w-4" />
                </Button>
              </MenuHandler>
              <MenuList>
                {priceOptions.map((option) => (
                  <MenuItem key={option.value} onClick={() => handlePriceChange(option)}>
                    {option.label}
                  </MenuItem>
                ))}
              </MenuList>
            ...
            <Menu>
              <MenuHandler>
                <Button variant="text" className="flex items-center gap-2 text-gray-700">
                  {selectedLocation} <ChevronDownIcon strokeWidth={2} className="h-4 w-4" />
                </Button>
              </MenuHandler>
              <MenuList>
                {locationOptions.map((option) => (
                  <MenuItem key={option.value} onClick={() => handleLocationChange(option)}>
                    {option.label}
                  </MenuItem>
                ))}
              </MenuList>
            ...
            <Menu>
              <MenuHandler>
                <Button variant="text" className="flex items-center gap-2 text-gray-700">
                  {selectedRating} <ChevronDownIcon strokeWidth={2} className="h-4 w-4" />
                </Button>
              </MenuHandler>
              <MenuList>
                {ratingOptions.map((option) => (
                  <MenuItem key={option.value} onClick={() => handleRatingChange(option)}>
                    {option.label}
                  </MenuItem>
                ))}
              </MenuList>
