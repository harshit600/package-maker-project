...
function DayWiseCabPricing({
  ...
  const handleDateRangeSelect = (value) => {
    if (!selectedCab || !selectedSeason) return;
    ...
    if (value.length === 2 && value[0] && value[1]) {
      ...
      const updatedSeasonDates = {
        ...seasonDates,
        [selectedCab]: {
          onSeason: [...(seasonDates[selectedCab]?.onSeason || [])],
          offSeason: [...(seasonDates[selectedCab]?.offSeason || [])],
        },
      };

      dateList.forEach((date) => {
        updatedSeasonDates[selectedCab].onSeason = updatedSeasonDates[
          selectedCab
        ].onSeason.filter((d) => d !== date);
        updatedSeasonDates[selectedCab].offSeason = updatedSeasonDates[
          selectedCab
        ].offSeason.filter((d) => d !== date);
      });

      if (selectedSeason === "on") {
        updatedSeasonDates[selectedCab].onSeason.push(...dateList);
      } else {
        updatedSeasonDates[selectedCab].offSeason.push(...dateList);
      }

      setSeasonDates(updatedSeasonDates);
      updateCabPayload(prices, updatedSeasonDates);
      setTimeout(() => setDateRange([null, null]), 500);
    }
  };
  ...
  return (
    <div className="p-4 mx-auto">
                      ...
                      <div>
                        ...
                        {selectedCabCard === cab.cabName && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              openCalendarWithSeason(cab.cabName, "on");
                            }}
                            className="w-full mt-2 bg-yellow-500 text-white py-2 rounded-lg hover:bg-yellow-600 transition-colors text-sm"
                          >
                            Set On Season Dates
                          </button>
                        )}
                      ...
                      <div>
                        ...
                        {selectedCabCard === cab.cabName && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              openCalendarWithSeason(cab.cabName, "off");
                            }}
                            className="w-full mt-2 bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600 transition-colors text-sm"
                          >
      ...
      {/* Calendar Modal */}
      {showCalendar && selectedCab && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl max-w-2xl w-full">
            <div className="flex justify-between mb-4">
              <div>
                <h3 className="text-xl font-semibold">
                  Set {selectedSeason === "on" ? "On" : "Off"} Season Dates
                </h3>
                <p className="text-sm text-gray-600 mt-1">
                  {selectedCab} - ₹
                  {selectedSeason === "on"
                    ? prices[selectedCab]?.onSeasonPrice
                    : prices[selectedCab]?.offSeasonPrice}{" "}
                  per day
                </p>
              </div>
              <button
