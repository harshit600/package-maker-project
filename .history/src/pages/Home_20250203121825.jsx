...
export default function Home() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const destinationsResponse = await fetch(
          `${config.API_HOST}/api/destination/getdestination`
        );
        ...
        // Extract unique user types from users data
        const uniqueUserTypes = [
          ...new Set(usersData.map((user) => user.userType)),
        ];
  ...
  const filteredPackages = packages.filter((pkg) =>
    pkg.packageName.toLowerCase().includes(packageFilter.toLowerCase())
  );

  const filteredUsers = users.filter((user) =>
    user.userType.toLowerCase().includes(userTypeFilter.toLowerCase())
  );
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex">
        ...
        {/* Users Section with Dynamic User Types */}
      ...
      <div className="bg-white rounded-xl p-6 shadow-lg">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">Destinations List</h2>
        {loading ? (
          ...
          <div className="overflow-y-auto" style={{ height: "300px" }}>
            <table className="w-full min-w-full">
              <thead className="sticky top-0 bg-white">
                <tr className="text-left border-b">
                  <th className="pb-3 px-4">Destination Name</th>
                  <th className="pb-3 px-4">Location</th>
              ...
              <tbody>
                {destinations.map((destination) => (
                  <tr key={destination._id} className="border-b hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <div className="font-medium">{destination.destinationName}</div>
                    </td>
                    <td className="py-3 px-4">{destination.location}</td>
                    <td className="py-3 px-4">
                      <div className="truncate max-w-xs">{destination.description}</div>
                    </td>
                    <td className="py-3 px-4">
