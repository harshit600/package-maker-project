...
function CabManager() {
  ...
  const handleDropDownChange = (field, value) => {
    setFormData((prev) => {
      ...
      // If changing cab type, update cab name options and reset cab name
      if (field === "cabType") {
        setCabNameOptions(carOptions[value] || []);
        newFormData.cabName = ""; // Reset cab name when type changes
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!formData.cabSeatingCapacity)
        errors.cabSeatingCapacity = "Seating capacity is required";
      if (!formData.cabLuggage)
        errors.cabLuggage = "Luggage capacity is required";
      if (formData.cabImages.length === 0)
        errors.cabImages = "At least one image is required";
