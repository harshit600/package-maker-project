...
function CabManager() {
  ...
  const handleImageUpload = async () => {
    try {
      ...
      if (files.length + formData.cabImages.length >= 7) {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.cabType || !formData.cabName || !formData.cabSeatingCapacity || !formData.cabLuggage) {
      alert('Please fill in all required fields');
      return;
    }

    // Format seating capacity to ensure it's in the correct format
    const formattedSeatingCapacity = formData.cabSeatingCapacity.toString().includes('Seater') 
      ? formData.cabSeatingCapacity 
      : `${formData.cabSeatingCapacity} Seater`;

    const payload = {
      _id: selectedCabId, // Include the ID in the payload for editing
    ...
    try {
      ...
      console.log('Cab ID:', selectedCabId);
      ...
      alert(`Failed to ${isEditing ? 'update' : 'create'} cab: ${error.message}`);
    }
  };
