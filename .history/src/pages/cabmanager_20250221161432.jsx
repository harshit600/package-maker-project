...
function CabManager() {
  ...
  const handleEditCab = (cab) => {
    ...
    console.log("Editing cab with ID:", cab.id);
  ...
  const handleSubmit = async () => {
    ...
    // Create the payload
    const payload = {
      id: isEditing ? selectedCabId : undefined,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || errorData.error || "Failed to save cab");
      }
  ...
  const fetchCabs = async () => {
    try {
      ...
      const data = await response.json();
      console.log("Fetched cabs data structure:", data); // Log the data structure
      setCabs(data.result);
    } catch (error) {
