...
function CabManager() {
  ...
  const handleEditCab = (cab) => {
    ...
    const seatingCapacity = cab.cabSeatingCapacity.split(" ")[0]; // Extract number from "X Seater"

    setFormData({
      cabType: cab.cabType || "",
      cabName: cab.cabName || "",
      cabImages: cab.cabImages || [],
      cabSeatingCapacity: seatingCapacity,
  ...
  const handleSubmit = async () => {
    ...
    const formattedSeatingCapacity = `${formData.cabSeatingCapacity} Seater`;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to save cab");
      }
