...
function CabManager() {
  ...
  const handleSubmit = async () => {
    ...
    const formattedSeatingCapacity = typeof formData.cabSeatingCapacity === 'number' 
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || `Failed to ${isEditing ? 'update' : 'create'} cab`);
      }
  ...
  const fetchCabs = async () => {
    try {
      ...
      const data = await response.json();
      console.log("Fetched cabs:", data.result);
      setCabs(data.result);
    } catch (error) {
