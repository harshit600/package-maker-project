...
function CabManager() {
  ...
  const seatingOptions = Array.from({ length: 47 }, (_, index) => {
    const seats = index + 4;
    return { label: `${seats} Seater`, value: seats };
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to create cab");
      }
      ...
      alert("Failed to create cab: " + error.message);
    }
  };
  ...
  const fetchCabs = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to fetch cabs");
      }
      ...
      alert("Error fetching cabs: " + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="shadow-md min-h-full p-2 m-2">
      ...
      <Modal show={isModalOpen} closeModal={closeModal} title="Add Cab">
        <div className="mb-4">
      ...
      <div className="flex flex-wrap justify-center mt-5">
        {loading ? (
          ...
          filteredCabs.map((cab, index) => (
            <CabCard key={index} cab={cab} onDelete={handleDeleteCab} />
          ))
        ) : (
