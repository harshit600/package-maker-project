...
function CabManager() {
  ...
  const fetchCabs = async () => {
    try {
      ...
      if (Array.isArray(data.result)) {
        setCabs(data.result);
        console.log("Setting cabs:", data.result);
      } else {
        console.error("Invalid cabs data format:", data);
        setCabs([]);
      }
    } catch (error) {
  ...
  useEffect(() => {
    console.log("Current cabs state:", cabs);
    console.log("Current filters:", filters);

    // Ensure cabs is an array before filtering
    const cabsArray = Array.isArray(cabs) ? cabs : [];
    let filtered = [...cabsArray];
    console.log("Initial filtered cabs:", filtered);

    if (filters.cabType) {
      filtered = filtered.filter((cab) => cab.cabType === filters.cabType);
      console.log("Filtered by cabType:", filtered);
    }
