...
function CabManager() {
  ...
  const fetchCabs = async () => {
    try {
      setLoading(true);
      const apiUrl = `${config.API_HOST}/api/cabs/getallcabs`;
      ...
      const cabsData = Array.isArray(data)
        ? data
        : data.cabs || data.result || [];
      console.log("Processed cabs data:", cabsData);
      ...
      console.error(
        ...
        error,
        "Full URL:",
        `${config.API_HOST}/api/cabs/getallcabs`
      );
      setCabs([]);
    } finally {
      setLoading(false);
