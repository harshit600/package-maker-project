...
const ActivitySlider = ({ isOpen, onClose, selectedDay, onActivitySelect }) => {
  ...
  const handleActivitySelect = useCallback(
    (activity) => {
      ...
      const activityWithDay = {
        ...
        location_site: activity.location || "Location",
        city: activity.city || "City",
        tags: activity.tags || "Activity",
        category_id: activity.category_id || "Category",
        imageUrl:
          activityImages[activity.id]?.[0] ||
          "https://placehold.co/300x200?text=Activity",
        quantity: 1, // Adding initial quantity
      };

      onActivitySelect(activityWithDay);
      onClose();
    },
    [selectedDay, onActivitySelect, activityImages, onClose]
  );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const [activitiesResponse, imagesResponse] = await Promise.all([
          fetch("https://backendactivity.plutotours.in/api/products"),
          fetch("https://backendactivity.plutotours.in/api/images"),
        ]);
  ...
  const filteredActivities = activities.filter((activity) => {
    ...
    const cityMatches = selectedDay?.selectedItinerary?.cityName?.toLowerCase() === activity.city?.toLowerCase();
    
    // Other existing filters
    const matchesCategory = selectedCategory === "Any" || activity.category_id === selectedCategory;
    const matchesPrice = parseInt(activity.price) <= priceRange[1];
    const matchesSearch = !searchQuery ||
      activity.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      activity.short_description.toLowerCase().includes(searchQuery.toLowerCase());

    return cityMatches && matchesCategory && matchesPrice && matchesSearch;
  });

  // Add a message when no activities are found for the selected city
  const noActivitiesMessage = selectedDay?.selectedItinerary?.cityName && filteredActivities.length === 0 ? (
    <div className="text-center py-8 text-gray-500">
      No activities found in {selectedDay.selectedItinerary.cityName}
    </div>
  ) : null;
    ...
    <div className="fixed inset-y-0 right-0 w-[800px] bg-white shadow-lg z-50 overflow-y-auto">
      <div className="p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">Add Activity Day wise</h2>
        ...
        <div className="text-sm text-gray-600 mb-4">
          {selectedDay?.selectedItinerary?.cityName && (
