  ...
  useEffect(() => {
    try {
      ...
      if (userStr) {
        ...
        // Normalize the userType check to handle both "Manager" and "manager" cases
        setCurrentUser({
          ...user,
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        >
          ...
          {/* Form Header */}
          <div className="bg-[rgb(45,45,68)] px-8 py-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div className="form-group">
                ...
                >
                  ...
                  {currentUser?.userType?.toLowerCase() === "Manager" && (
     ...
     <div className="form-group">
               ...
               {/* Team Leader Name - Show only if designation is executive */}
               {formData.designation === "executive" && formData.companyName && (
                 <div className="form-group">
                   <label className="block text-sm font-semibold text-gray-700 mb-2">
                     Team Leader Name
                   </label>
                   ...
                   >
                     ...
                     {getTeamLeadersByCompany(formData.companyName).map((teamLeader) => (
                       <option key={teamLeader._id} value={teamLeader.firstName + " " + teamLeader.lastName}>
                         {teamLeader.firstName} {teamLeader.lastName}
                       </option>
                     ))}
                   </select>
                   {getTeamLeadersByCompany(formData.companyName).length === 0 ? (
                     <p className="mt-1 text-sm text-orange-600">
                       No team leaders found for {formData.companyName}. Please add a team leader first.
                     </p>
                   ) : (
                     <p className="mt-1 text-sm text-green-600">
                       {getTeamLeadersByCompany(formData.companyName).length} team leader(s) available
                     </p>
                   )}
                 </div>
               )}

               {/* Manager Name - Show for executives and team leaders */}
               {(formData.designation === "executive" || formData.designation === "team leader") && formData.companyName && (
                 <div className="form-group">
                   <label className="block text-sm font-semibold text-gray-700 mb-2">
                     Manager Name
                   </label>
                   ...
                   >
                     ...
                     {getManagersByCompany(formData.companyName).map((manager) => (
                       <option key={manager._id} value={manager.firstName + " " + manager.lastName}>
                         {manager.firstName} {manager.lastName}
                       </option>
                     ))}
                   </select>
                   {getManagersByCompany(formData.companyName).length === 0 ? (
                     <p className="mt-1 text-sm text-orange-600">
                       No managers found for {formData.companyName}. Please add a manager first.
                     </p>
                   ) : (
                     <p className="mt-1 text-sm text-green-600">
                       {getManagersByCompany(formData.companyName).length} manager(s) available
                     </p>
                   )}
                 </div>
               )}
            ...
            <div className="mt-8 flex justify-end space-x-4">
              ...
              <button
                type="submit"
                className="px-6 py-3 bg-[rgb(45,45,68)] text-white rounded-lg hover:bg-[rgb(55,55,78)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200"
              >
                Create User
              </button>
            </div>
