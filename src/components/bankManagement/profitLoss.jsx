...
function ProfitLoss() {
  ...
  const getFilteredData = () => {
    ...
    return serviceReport.filter(operation => {
      if (!operation.updatedAt) return true;
      
      const updatedDate = new Date(operation.updatedAt);
      const updatedMonth = updatedDate.getMonth() + 1; // getMonth() returns 0-11
      const updatedYear = updatedDate.getFullYear();
      
      // Month filter
      if (selectedMonth && selectedMonth !== updatedMonth.toString()) return false;
      
      // Year filter
      if (selectedYear && selectedYear !== updatedYear.toString()) return false;
      
      // Date range filter
      if (startDate && updatedDate < new Date(startDate)) return false;
      if (endDate && updatedDate > new Date(endDate)) return false;
  ...
  const calculateTotals = () => {
    ...
    const totals = filteredData.reduce((acc, operation) => {
      console.log(acc);
      // Base costs from totals
      const activitiesCost = operation.totals?.activitiesCost || 0;
      const hotelCost = operation.totals?.hotelCost || 0;
      const transferCost = operation.totals?.transferCost || 0;
  ...
  const getCostBreakdown = (totals) => {
    if (!totals) return { hotelCost: 0, transferCost: 0, activitiesCost: 0 };
    
    const baseCost = (totals.hotelCost || 0) + (totals.transferCost || 0) + (totals.activitiesCost || 0);
    if (baseCost === 0) return { hotelCost: 0, transferCost: 0, activitiesCost: 0 };
    
    return {
      hotelCost: ((totals.hotelCost || 0) / baseCost) * 100,
      transferCost: ((totals.transferCost || 0) / baseCost) * 100,
      activitiesCost: ((totals.activitiesCost || 0) / baseCost) * 100
    };
  };
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className=" divide-y divide-gray-100">
                  ...
                  const costBreakdown = getCostBreakdown(operation.totals);
                                  ...
                                  <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                      <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                                        ...
                                        <div className="text-lg font-bold text-blue-900 mt-1">{formatCurrency(operation.totals?.hotelCost || 0)}</div>
                                      </div>
                                      ...
                                      <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                                        ...
                                        <div className="text-lg font-bold text-green-900 mt-1">{formatCurrency(operation.totals?.transferCost || 0)}</div>
                                      </div>
                                      ...
                                      <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                                        ...
                                        <div className="text-lg font-bold text-purple-900 mt-1">{formatCurrency(operation.totals?.activitiesCost || 0)}</div>
                                      </div>
