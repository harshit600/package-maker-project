...
const PackageCreation = ({ initialData, isEditing, editId }) => {
  ...
  // Add new state for package form data
  const [packageFormData, setPackageFormData] = useState(null);

  const handlePackageFormData = (formData) => {
    // Update the new state with form data
    setPackageFormData(formData);
    
    // Immediate logging of received data
    console.log('Received Form Data:', formData);
  };

  // Add useEffect to monitor packageFormData changes
  useEffect(() => {
    // Log whenever packageFormData changes
    console.log('PackageFormData State Updated:', packageFormData);
  }, [packageFormData]);

  // Add useEffect to fetch data from /api/add/get
  useEffect(() => {
  ...
  // Debug log to check component renders
  useEffect(() => {
    console.log('PackageCreation Component Mounted');
  }, []);
...
useEffect(()=>{
console.log("hfghgfhgfh",packageFormData)
},[]);
  return (
        ...
        {activeTab === "Package" && (
          <div className="step-1">
            <PackageForm
              ...
              uploading={uploading}
              isLoading={isLoading}
              onFormDataChange={handlePackageFormData}
            />
          </div>
        )}
        {activeTab === "Cabs" && (
