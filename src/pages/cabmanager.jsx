...
function CabManager() {
  ...
  const [cabs, setCabs] = useState([]); // This will store and display all cabs
  ...
  const handleSubmit = async () => {
    ...
    const errors = [];
    if (!formData.cabType) errors.push("Cab Type");
    if (!formData.cabName) errors.push("Cab Name");
    if (!formData.cabSeatingCapacity || formData.cabSeatingCapacity === "Select Seating Capacity") errors.push("Seating Capacity");
    if (!formData.cabLuggage) errors.push("Luggage Capacity");
    ...
    try {
      ...
      console.error("Full error details:", {
        ...
        selectedCabId,
        isEditing,
        url: isEditing ? `${config.API_HOST}/api/cabs/editcab/${selectedCabId}` : `${config.API_HOST}/api/cabs/createcab`,
  ...
  const fetchCabs = async () => {
    try {
      ...
      setCabs(data.result); // Set the segregated cabs data
    } catch (error) {
    ...
    if (window.confirm("Are you sure you want to delete this cab?")) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || "Failed to delete cab");
        }

        await fetchCabs(); // Refresh the cabs list
      } catch (error) {
        console.error("Error deleting cab:", error);
        alert(`Failed to delete cab: ${error.message}`);
      }
    }
  };
    ...
    <div className="shadow-md min-h-full p-2 m-2">
      ...
      <div className="flex flex-wrap justify-center mt-5">
        {loading ? (
          ...
          <div className="text-gray-500 text-center py-8">No cabs found</div>
        )}
      </div>
    </div>
  );
